...
void scp::Model::Initialize(const std::string filename) {
  ClearData();
  ParseFile(filename);
  TranslateToOrigin();
  SaveVertices();
...
void scp::Model::RestoreVertices() {
...
void scp::Model::ApplyAffine(AffineData& data) {
...
void scp::Model::AffineMove(double ax, double ay, double az) {
  if (!ax && !ay && !az) return;
...
void scp::Model::AffineRotateX(double angle) {
  if (!angle) return;
...
void scp::Model::AffineRotateY(double angle) {
  if (!angle) return;
  double cos_angle = cos(angle), sin_angle = sin(angle);
  for (size_t i = 0; i < vertices_.size(); i += 3) {
...
void scp::Model::AffineRotateZ(double angle) {
  if (!angle) return;
  double cos_angle = cos(angle), sin_angle = sin(angle);
  for (size_t i = 0; i < vertices_.size(); i += 3) {
    double temp_x = vertices_[i] - center_x_,
...
void scp::Model::AffineScale(double k) {
  k = (k == 0) ? 1 : k;
  for (size_t i = 0; i < vertices_.size(); i += 3) {
    vertices_[i] *= k;
    vertices_[i + 1] *= k;
...
size_t scp::Model::GetPolygonsEdgesCount() const {
  return polygons_edges_count_;
}

size_t scp::Model::GetVerticesCount() const { return vertices_.size() / 3; }

size_t scp::Model::GetPolygonsCount() const { return polygons_.size(); }

const std::vector<int>& scp::Model::GetPolygons() const { return polygons_; }

const std::vector<double>& scp::Model::GetVertices() const { return vertices_; }

double scp::Model::GetCenterX() const { return center_x_; }
double scp::Model::GetCenterY() const { return center_y_; }
double scp::Model::GetCenterZ() const { return center_z_; }

bool scp::Model::Empty() const { return is_empty_; }

void scp::Model::NormalizeAngle(double& angle) {
  while (angle < 0) angle += 360 * 16;
  while (angle > 360) angle -= 360 * 16;
}

void scp::Model::ClearData() {
  is_empty_ = true;
  vertices_.clear();
  polygons_.clear();
  min_point_ = {0, 0, 0};
...
void scp::Model::ParseFile(std::string filename) {
...
bool scp::Model::ParseVertices(std::istringstream& iss) {
  Vertex3d point;
  bool res = false;
...
bool scp::Model::ParsePolygons(std::istringstream& iss) {
...
void scp::Model::UpdateMinMaxPoints(Vertex3d point) {
...
void scp::Model::TranslateToOrigin() {
...
void scp::Model::SaveVertices() { vertices_origin_ = vertices_; }
